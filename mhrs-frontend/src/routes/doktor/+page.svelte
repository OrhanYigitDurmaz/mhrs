<script lang="ts">
	...
	async function handleAddHour() {
		...
		try {
			...
			await loadData(doktorId);
			hourForm = { gun: 'Pazartesi', saat_bas: '09:00', saat_bit: '17:00' };
...
{:else}
	...
	<div class="mb-8">
		...
		<p class="mt-2 text-slate-600">Çalışma saatlerinizi ve randevu slotlarınızı yönetin.</p>
	</div>
	...
	{:else}
		<Tabs value={activeTab}>
			<TabsList class="mb-6">
				<TabsTrigger onclick={() => activeTab = 'slots'} value="slots">Randevu Slotları</TabsTrigger>
				...
				<div class="grid gap-6 lg:grid-cols-2">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Çalışma saatlerinize göre otomatik olarak randevu slotları oluşturun.
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Button onclick={handleGenerateSlots} disabled={isGenerating} class="w-full">
								{#if isGenerating}
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>Müsait olan yaklaşan slotlar.</CardDescription>
						</CardHeader>
						<CardContent>
							{#if upcomingSlots.length === 0}
								<p class="py-4 text-center text-sm text-slate-600">Uygun slot bulunamadı.</p>
							{:else}
								<div class="space-y-2">
											...
											<div class="flex items-center gap-3">
												...
												<div>
													<p class="text-sm font-medium">
														{new Date(slot.tarih).toLocaleDateString('tr-TR', {
															month: 'short',
															day: 'numeric'
														})}
													</p>
													<p class="text-xs text-slate-600">{slot.saat}</p>
												</div>
											</div>
											<Badge variant="outline">Müsait</Badge>
										</div>
									{/each}
								</div>
							{/if}
					...
					<CardContent>
						<div class="overflow-x-auto">
							<table class="w-full text-sm">
								<thead>
									<tr class="border-b">
										<th class="py-3 text-left font-medium">Tarih</th>
										<th class="py-3 text-left font-medium">Saat</th>
										<th class="py-3 text-left font-medium">Durum</th>
										<th class="py-3 text-right font-medium">İşlem</th>
									</tr>
								</thead>
								<tbody>
									{#each slots.slice(0, 20) as slot}
										<tr class="border-b">
											...
											<td class="py-3">
												<Badge variant={slot.dolu > 0 ? 'destructive' : 'default'}>
													{slot.dolu > 0 ? 'Dolu' : 'Müsait'}
												</Badge>
											</td>
						...
						<Card>
							<CardHeader class="flex flex-row items-center justify-between">
								<div>
									...
									<CardDescription>Haftalık çalışma saatlerinizi buradan yönetin.</CardDescription>
								</div>
								<Button onclick={() => showAddHourForm = !showAddHourForm} size="sm">
									<Plus class="mr-2 h-4 w-4" />
									Saat Ekle
								</Button>
							</CardHeader>
							<CardContent>
								{#if workingHours.length === 0}
									<div class="py-8 text-center">
										<Clock class="mx-auto h-12 w-12 text-slate-300" />
										<p class="mt-4 text-slate-600">Henüz çalışma saati tanımlanmamış.</p>
									</div>
								{:else}
									<div class="space-y-4">
										{#each DAYS as day}
											{#if workingHours.filter(h => h.gun === day).length > 0}
												<div>
													<h4 class="mb-2 text-sm font-medium text-slate-700">{day}</h4>
													<div class="space-y-2">
						...
						<Card>
							...
							<CardContent>
								<form onsubmit={(e) => { e.preventDefault(); handleAddHour(); }} class="space-y-4">
									<div class="space-y-2">
										<label class="text-sm font-medium">Gün</label>
										<select
									...
									<div class="flex gap-2">
										<Button type="button" variant="outline" onclick={() => showAddHourForm = false} class="flex-1">
											İptal
										</Button>
										<Button type="submit" disabled={isAddingHour} class="flex-1">
											{#if isAddingHour}
												<Loader2 class="mr-2 h-4 w-4 animate-spin" />
											{/if}
											Ekle
